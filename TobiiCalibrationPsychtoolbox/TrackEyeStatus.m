...
global EYETRACKER KEYBOARD KEYID EXPWIN BLACK CENTER
...
while 1
    ...
    if (~LValid && ~RValid)
        ...
        Screen('FillRect',EXPWIN,[0 0 100], figloc);
        EyeUpdateMsg = ['Left Eye ' num2str(GazeData(end).LeftEye.GazePoint.InUserCoordinateSystem)];
    ...
    if LValid || RValid
        if LValid
            scaled_Lx = (150+GazeData(end).LeftEye.GazePoint.InUserCoordinateSystem(1))/300; %Just play with these until they stay mostly in the blue field, Tobii's user coordinate system is weird. 
            scaled_Ly = 1 - (500+GazeData(end).LeftEye.GazePoint.InUserCoordinateSystem(2))/1000;
        ...
        if RValid
            scaled_Rx = (100+GazeData(end).RightEye.GazePoint.InUserCoordinateSystem(1))/200;
            scaled_Ry = 1 - (400+GazeData(end).RightEye.GazePoint.InUserCoordinateSystem(2))/800;
